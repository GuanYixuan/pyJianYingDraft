...
### 批量导出
> ℹ 剪映7+版本隐藏了部分控件，导出自动化会在常规UI控件不可用时尝试快捷键/坐标等兜底方案；不同剪映版本界面差异较大，建议先用一个短草稿试跑。
