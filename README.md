...
### 模板模式
为了保留部分复杂特性（文本特效、复合片段...），可以加载一个已有的剪映草稿作为模板，然后**替换其中音视频片段的素材**。

目前提供了**两种替换素材的方式**
- [根据名称](#根据名称替换素材)：直接替换素材本身，自然影响所有引用该素材的片段
- [根据片段](#根据片段替换素材)：替换某个特定片段的素材，同时重新选取其引用的素材范围
...
#### 加载模板
加载模板文件（`draft_content.json`）创建`Script_file`对象可利用`load_template`方法：

```python
import pyJianYingDraft as draft
script = draft.Script_file.load_template("<你的草稿文件夹>/draft_content.json")
```

为了最大限度地兼容模板中的复杂特性，**导入的轨道与pyJianYingDraft创建的轨道是分离开的**，具体地讲：

- 在导入的轨道上，**只能够替换其中音视频片段的素材**
- 不能在导入的轨道上添加片段，转场、淡入淡出等
- 仍然可以**创建新的轨道，并在其上添加片段等**，就像非模板模式一样
...
此方法分为两步：**选取轨道**和**替换素材**，以上方音频素材的替换为例：
```python
from pyJianYingDraft import trange, Shrink_mode, Extend_mode

imported_track = script.get_imported_track(
...
script.replace_material_by_seg(
    imported_track, 0, new_material,       # 选取轨道上下标为0的片段，也即第一个片段
...
从例子中可见，此替换方法可能会造成片段的时长变化，故需要利用`handle_shrink`和`handle_extend`参数指定片段在缩短和延长时的处理方式。
